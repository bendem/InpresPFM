 ...
 * @author Bears
 */
public class DisplayReservation extends BodyTagSupport {
    ...
    private void writeTagBodyContent(JspWriter out, BodyContent bodyContent) throws IOException {
        double total = 0;
        Map<Date, Integer> reservation = (Map<Date, Integer>)pageContext.getSession().getAttribute(bodyContent.getString());
        if(reservation == null)
            return;
        try {
            for (Map.Entry i : reservation.entrySet()){
                Reservation item = database.table(Reservation.class).byId(i.getKey()).get().get();
                out.write("Date: " + item.getReservationDay().toString() + "<br>" +
                    "Price/u: 10€<br>" +
                    "Quantity: " + i.getValue() + "<br>" +
                    "Sub-total: " + ((int)i.getValue())*10 + "<br><br>"
                );
                total = total + ((int)i.getValue())*10;
            }
        } catch (InterruptedException | ExecutionException ex) {
            Logger.getLogger(DisplayCart.class.getName()).log(Level.SEVERE, null, ex);    
        }
        pageContext.setAttribute("totalCart", (double)pageContext.getAttribute("totalCart") + total);
    }
    ...
    private void handleBodyContentException(Exception ex) throws JspException {
        ...
        throw new JspException("Error in DisplayReservation tag", ex);
    }
